...
An interesting property of XOR is that `a ^ b ^ a = b`, which means that repeatedly XORing with any hash will still result in effectivley XORing by that hash either once or zero times. Since we know all the hashes that can every possibly be XORed together, we can generate all the possible keys by XORing the hashes together in every possible combination, i.e. total of $\sum\limits_{x=0}^{20} {20 \choose x} = 1048576$ combinations, which is a small enough number to brute force by trying to decrypt with each key.
...
Looking at the hexdump (`xxd kpa.apk | tail`) at the end of hte file, we find the EOCD record, marked by its signature (0x06054b50), starting at offset 0x002b0d49:
...
To be honest, I didn't really know how to approach this problem because it seemd like a RE challenge. But looking at the defined strings in [Ghidra](https://github.com/NationalSecurityAgency/ghidra), there were some interesting strings like "Error launching installer" and "Error writing temporary file. Make sure your temp folder is valid.", which led me to stare at the %TEMP% folder hoping maybe the battleship map will appear there.

Unfortunately, that was not the case. Fortunately, there was indeed useful things in the %TEMP% folder. We discover a folder seemingly containing the program files of the game, which contained license files for Chromium and Electron. Hm, an maybe electron app? Sure enough, an [app.asar](./tisc-2023/4-app.asar) will be found in the resources sub-folder. The file is an electron archive, which we can use the [asar](https://github.com/electron/asar) CLI to unpack, giving us the source code of the app.
...
Since we now have a constant map, we can bruteforce each cell to find all cells that gives us a "boom". I used the following script in the console to achieve this:
...
Nowe, we enter the password in the portal and get shown page with a "Welcome!" hyperlink that is a [Discord invite](https://discord.gg/2cyZ6zpw7J). Opening the actual response from the server, we get the following:
...
From having worked with Discord bots in the past and the given comment about a "token", we deduce that this is likely a Discord bot token. Since I did not really want to code, I tried finding Discord "bot clients" that attempts to replicate Discord clients while using a bot token to login. I found [this](https://github.com/aiko-chan-ai/DiscordBotClient), which worked. Using this would be able to give us the solution directly after staring at the Audit Log for long enough. However, let us go through the full process of solving this challenge :smiley_cat:.
...
This permissions likely refer to the permissions of the bot whose token is given to us, and we can confirm this by checking if we can view the Audit Log, which works. Using the following script, we can print all invites that have been created that are still recorded in the Audit Log:
...
Then, we access the [site for temporary credentials](https://13.213.29.24/) using `curl -k https://13.213.29.24/ --cert client.crt --key client.key`. This gets us our temporary AWS credentials with an ouput similar to the following:
...
First, I tried to use [enumerate-iam](https://github.com/andresriancho/enumerate-iam) and `iam__bruteforce_permissions` in [Pacu](https://github.com/RhinoSecurityLabs/pacu). This did not really yield much useful results since neither displayed all the permissions we had, possibly because some permissions are restricted to certain resources, and bruteforcing with a generic dummy resource will not work. However, there are some useful permissions, and two other interesting ones:
...
I then wrote a (somewhat working) [script](./tisc-2023/7-get_roles_and_policies.py) to attempt to get all (both inline and attached managed policies) for each of these roles. The output files of the script can be found [here](./tisc-2023/7-roles/). It is also at this point when I realised that I can use `aws sts get-caller-identity` to find my own username, which I can use in `aws iam list-user-policies` and `aws iam list-attached-user-policies`, with `--user-name agent-b92481fa329a4d32a4c848ea8a225020`. There was no result for `list-user-policies`, but we have one attached policy:
...
It is here where we can see that the first part of the flag is stored in the environment variable `flag1`, which is retrieved from the parameter store `/devsecmeow/build/password`. The `rawr.zip` which we conrol is used as the folder for the Terraform project, which is initialised and planned.
...
This will get a script for a reverse shell and run it on the sever. On our machine, we listen using `nc -l 1337`, and use [ngrok](https://ngrok.com/) to get a public TCP tunnel. Now that we have a reverse shell, we can print the flag. (Alternatively, for this part of the challenge, we can simply post the environment variable to a request bin by using `curl`.)
...
Since the flag is in a HTMl file, we can try to see if we can connect to the public IP address with HTTP. However, there is no response.
...
Since we know that AWS credentials is usually in plaintext in the `~/.aws/credentials` file (and from the Dockerfile we know that it is in the `/root/.aws/` folder in this case), we try to pass that as the `viewType` value. We get the following error which leaks the AWS credentials:
...
After search for and reading more about WebAssembly PWN, we find [a writeup](https://github.com/Charo-IT/CTF/blob/master/2018/codeblue_quals/writeup.md#secret-mailer-service-20-pwn) that describes using buffer overflows to control function calls.
...
From the related writeup linked above, we figure that we are able to overflow and change the `call_indirect` to call `load_query` instead of `is_blacklisted` by changing the index of fuction to be called to 2. I did this by gradually increasing the length of the `username` submitted to the Lambda function until an error occurs, then increasing until a `table index is out of bounds` first occurs, then change the last byte of the `username` to `\x02`. Now, even if our `username` contains non-alphabet characters, the Lmabda function will still return a SQL query statement:
